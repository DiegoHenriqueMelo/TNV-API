...
export const create = async (login: {
  ...
  try {
    ...
    const tipo = TipoUsuario[login.tipo.toUpperCase() as keyof typeof TipoUsuario];
    if (!tipo) throw new Error(`Tipo de usuário inválido: ${login.tipo}. Valores aceitos: ${Object.keys(TipoUsuario).join(", ")}`);

    const user = new User(login.nome, login.email, passHased, tipo);
